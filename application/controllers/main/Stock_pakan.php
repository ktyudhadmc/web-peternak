...
class Stock_pakan extends CI_Controller
{
    ...
    where a.id_kandang in (SELECT id from kandang WHERE user_id = '{$id_user}') and a.tanggal_kandang_activity_log BETWEEN '{$tanggal_mulai}' and '{$tanggal_akhir}' and type_sapronak = 'pakan' and method_sapronak = 'use'
    ...
    where id_kandang in (SELECT id from kandang where user_id = '{$id_user}') and tanggal_kandang_activity_log BETWEEN '{$tanggal_mulai}' and '{$tanggal_akhir}' and type_sapronak = 'pakan' and method_sapronak = 'use'";
        $getdatakonsumsipakan = $this->MData->customresult($querykonsumsipakan);
        echo json_encode($getdatakonsumsipakan);
    ...
    {
        $stock_in = "IFNULL((SELECT SUM(qty) AS tot_qty FROM `kandang_activity_stock` WHERE tanggal <= '{$tanggal_akhir}' and method_stock = 'in' and type_stock = 'pakan' AND id_user = '{$id_user}' AND id_this = b.id),0)";
        $stock_mutasi = "IFNULL((SELECT SUM(qty) AS tot_qty FROM `kandang_activity_stock` WHERE tanggal <= '{$tanggal_akhir}' and method_stock = 'out' and type_stock = 'pakan' AND id_user_to = '{$id_user}' AND id_this = b.id),0)";
        $stock_out = "IFNULL((SELECT SUM(qty) AS tot_qty FROM `kandang_activity_stock` WHERE tanggal <= '{$tanggal_akhir}' and method_stock = 'out' and type_stock = 'pakan' AND id_user = '{$id_user}' AND id_this = b.id),0)";
    ...
    FROM
    pakan AS b
    LEFT JOIN `kandang_activity_log_sapronak` AS a ON b.id = a.id_this and a.id_kandang IN (SELECT id FROM kandang WHERE user_id = '{$id_user}' and stock = 'lokasi') AND a.type_sapronak = 'pakan' AND a.method_sapronak = 'use' and a.tanggal_kandang_activity_log <= '{$tanggal_akhir}'
    GROUP BY
    b.id
    ORDER BY `b`.`brand` ASC";
    ...
    WHERE id_user = '{$id_user}' and type_stock = 'pakan' and method_stock = 'in' or id_user_to = '{$id_user}' and type_stock = 'pakan'";
